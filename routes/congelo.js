...
// Route GET pour récupérer les produits d'un utilisateur spécifique dans le congélateur
router.get('/:userId', async (req, res) => {
    const userId = req.params.userId; // Récupération de l'ID utilisateur depuis les paramètres
    
    try {
        // Recherche des produits dans la base de données pour un utilisateur et un lieu de stockage "Congelo"
        const products = await Product.find({ user: userId, storagePlace: "Congelo" });
